...
)
{
  ...
  /* No matter if success or failure, the subscriptionId in the response is always the one in the request */
  ...
  if (sub.isEmpty())
  {
    std::string errorString = std::string("subscriptionId: /") + requestP->subscriptionId.get() + "/";

    reqSemGive(__FUNCTION__, "ngsi10 unsubscribe request (no subscriptions found)", reqSemTaken);

    responseP->statusCode.fill(SccContextElementNotFound, errorString);
    responseP->oe.fill(SccContextElementNotFound, ERROR_DESC_NOT_FOUND_SUBSCRIPTION, ERROR_NOT_FOUND);
